...
describe('+ FilesController', () => {
  const rootDir = `${process.env.FOLDER_PATH || ''}`.trim().length > 0
  ...
  before(function (done) {
    this.timeout(10000);
    emptyDatabaseCollections(() => signUp(sampleUser, () => signIn(sampleUser, done)));
    emptyFolder(rootDir);
  });
  ...
  describe('+ POST: /files', () => {
    ...
    it('+ Fails if data is missing and type is not a folder', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({ name: sampleFiles[0].name, type: sampleFiles[0].type })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            return done(requestErr);
    ...
    it('+ Fails if unknown parentId is set', function (done) {
      ...
      request.post('/files')
        ...
        .send({
          ...
          type: sampleFiles[0].type,
          data: sampleFiles[0].b64Data(),
          parentId: 55,
        })
        .expect(400)
        .end((requestErr, res) => {
    ...
    it('+ Succeeds for valid values of a file', function (done) {
      ...
      request.post('/files')
        ...
        .end((requestErr, res) => {
          ...
          expect(res.body.name).to.eql(sampleFiles[0].name);
          expect(res.body.type).to.eql(sampleFiles[0].type);
          expect(res.body.isPublic).to.eql(false);
          expect(res.body.parentId).to.eql(0);
          sampleFiles[0].id = res.body.id;
          done();
        });
    });
    ...
    it('+ Succeeds for valid values of a folder', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({
          name: sampleFiles[1].name,
          type: sampleFiles[1].type,
          isPublic: true,
          parentId: 0,
        })
        .expect(201)
        .end((requestErr, res) => {
          ...
          expect(res.body.id).to.exist;
          expect(res.body.userId).to.exist;
          expect(res.body.name).to.eql(sampleFiles[1].name);
          expect(res.body.type).to.eql(sampleFiles[1].type);
          expect(res.body.isPublic).to.eql(true);
          expect(res.body.parentId).to.eql(0);
          sampleFiles[1].id = res.body.id;
          done();
        });
    });
    ...
    it('+ Fails if parentId is set and is not of a folder or 0', function (done) {
      ...
      request.post('/files')
        .set('X-Token', token)
        .send({
          name: sampleFiles[2].name,
          type: sampleFiles[2].type,
          data: sampleFiles[2].b64Data(),
          parentId: sampleFiles[0].id,
        })
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
    ...
    it('+ Succeeds if parentId is set and is of a folder', function (done) {
      ...
      request.post('/files')
        ...
        .end((requestErr, res) => {
          ...
          expect(res.body.id).to.exist;
          expect(res.body.userId).to.exist;
          expect(res.body.name).to.eql(sampleFiles[2].name);
          expect(res.body.type).to.eql(sampleFiles[2].type);
          expect(res.body.isPublic).to.eql(false);
          expect(res.body.parentId).to.eql(sampleFiles[1].id);
          sampleFiles[2].id = res.body.id;
          done();
        });
    });
  });
  ...
  describe('+ GET: /files/:id', () => {
    ...
    it('+ Succeeds if file is linked to user', function (done) {
      this.timeout(5000);
      request.get(`/files/${sampleFiles[0].id}`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
          ...
          expect(res.body.id).to.exist;
          expect(res.body.userId).to.exist;
          expect(res.body.id).to.eql(sampleFiles[0].id);
          expect(res.body.name).to.eql(sampleFiles[0].name);
          expect(res.body.type).to.eql(sampleFiles[0].type);
          expect(res.body.isPublic).to.eql(false);
          expect(res.body.parentId).to.eql(0);
  ...
  describe('+ GET: /files', () => {
    ...
    it('+ Fetches first page with no page query', function (done) {
      ...
      request.get('/files')
        ...
        .end((requestErr, res) => {
          ...
          expect(res.body.some((file) => file.name === sampleFiles[0].name)).to.be.true;
          expect(res.body.some((file) => file.name === sampleFiles[1].name)).to.be.true;
          done();
        });
    });
    ...
    it('+ Fetches first page with no page query and parentId', function (done) {
      this.timeout(5000);
      request.get(`/files?parentId=${sampleFiles[1].id}`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
          ...
          expect(res.body.some((file) => file.name === sampleFiles[2].name)).to.be.true;
          done();
        });
    });
    ...
    it('+ Returns empty list for a parentId of a file', function (done) {
      this.timeout(5000);
      request.get(`/files?parentId=${sampleFiles[0].id}`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ PUT: /files/:id/publish', () => {
    ...
    it('+ Succeeds if file is linked to user', function (done) {
      this.timeout(5000);
      request.put(`/files/${sampleFiles[0].id}/publish`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ PUT: /files/:id/unpublish', () => {
    ...
    it('+ Succeeds if file is linked to user', function (done) {
      this.timeout(5000);
      request.put(`/files/${sampleFiles[0].id}/unpublish`)
        .set('X-Token', token)
        .expect(200)
        .end((requestErr, res) => {
          if (requestErr) {
  ...
  describe('+ GET: /files/:id/data', () => {
    ...
    it('+ Fails if the file is not public and not requested by the owner', function (done) {
      request.get(`/files/${sampleFiles[0].id}/data`)
        .expect(404)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Succeeds if the file is not public but requested by the owner', function (done) {
      request.get(`/files/${sampleFiles[0].id}/data`)
        ...
        .end((err, res) => {
          ...
          expect(res.text).to.eql(sampleFiles[0].data);
          done();
        });
    });
    ...
    it('+ Fails if the file is a folder', function (done) {
      this.timeout(5000);
      request.get(`/files/${sampleFiles[1].id}/data`)
        .expect(400)
        .end((requestErr, res) => {
          if (requestErr) {
            console.error(requestErr);
    ...
    it('+ Succeeds if the file is not public but requested by the owner [alt]', function (done) {
      request.get(`/files/${sampleFiles[2].id}/data`)
        .set('X-Token', token)
        .expect(200)
        .end((err, res) => {
          if (err) {
          ...
          expect(res.text).to.eql(sampleFiles[2].data);
          done();
        });
    });
